"""Specialized vector store for government schemes using ChromaDB."""
...
class GovernmentSchemesVectorStore:
    """Specialized vector store for government schemes using ChromaDB."""
    
    def __init__(self, config: RAGConfig = None):
        """Initialize the government schemes vector store.
    ...
    def _initialize_store(self) -> None:
        """Initialize the ChromaDB client and government schemes collection."""
        try:
            ...
            # Get or create government schemes collection
            self.collection = self.client.get_or_create_collection(
                name="government_schemes",
                metadata={
                    "description": "Government scheme documents with embeddings",
                    "embedding_dimensions": self.config.embedding_dimensions,
                    "document_type": "government_schemes"
                }
            )
            
            logger.info(f"Government schemes vector store initialized at {persist_path}")
            logger.info(f"Collection: {self.collection.name}")
            
        except Exception as e:
            logger.error(f"Failed to initialize government schemes vector store: {e}")
            raise
    
    def store_chunks(self, chunks: List[AgriculturalChunk]) -> None:
        """Store government scheme chunks in the vector store.
        
        Args:
            chunks: List of government scheme chunks to store
        ...
        try:
            ...
            logger.info(f"Successfully stored {len(ids)} government scheme chunks in vector store")
            
        except Exception as e:
            logger.error(f"Failed to store government scheme chunks: {e}")
            raise
    ...
    def similarity_search(
        ...
        """Perform similarity search in the government schemes vector store.
        ...
        try:
            ...
            logger.debug(f"Retrieved {len(formatted_results)} results from government schemes similarity search")
            return formatted_results
            
        except Exception as e:
            logger.error(f"Failed to perform government schemes similarity search: {e}")
            raise
    ...
    def _build_where_clause(self, filter_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in filter_dict.items():
            if isinstance(value, list):
                # Handle list values (e.g., scheme names)
                if value:
    ...
    def get_collection_info(self) -> Dict[str, Any]:
        """Get information about the government schemes collection.
        ...
        try:
            ...
            logger.error(f"Failed to get government schemes collection info: {e}")
            return {"error": str(e)}
    
    def clear_collection(self) -> None:
        """Clear all data from the government schemes collection."""
        if not self.collection:
            logger.warning("Collection not initialized, cannot clear")
            return
        
        try:
            self.collection.delete(where={})
            logger.info("Government schemes collection cleared")
        except Exception as e:
            logger.error(f"Failed to clear government schemes collection: {e}")
            raise
